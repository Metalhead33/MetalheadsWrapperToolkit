...
private:
	...
	sSimpleResamplerImpl pimpl;
public:
	~SimpleResampler() = default;
	SimpleResampler();
	SimpleResampler(float* dataIn, long input_frames, float* dataOut, long outputFrames, double src_ratio);
	...
	void set(const float* data_in, float* data_out, long input_frames, long output_frames, double ratio) const;
};
...
private:
	...
	sFullResamplerImpl pimpl;
public:
	~FullResampler() = default;
	FullResampler();
	FullResampler(int channels, ResampleType converterType = ResampleType::SINC_FASTEST);
...
private:
	...
	sCallbackResamplerImpl pimpl;
public:
	virtual ~CallbackResampler() = default;
	CallbackResampler();
	CallbackResampler(CallbackResampler &&mov);
	void operator=(CallbackResampler&& mov);
	CallbackResampler(samplerate_callback func, ResampleType converter_type, int channels, void* userdata);
	CallbackResampler(CallbackInterfaceBase* interface, ResampleType converter_type, int channels);
...
class CallbackInterface : public CallbackInterfaceBase, public CallbackResampler
{
public:
	virtual ~CallbackInterface() = default;
	CallbackInterface(int channels = 1, ResampleType converter_type = ResampleType::SINC_FASTEST);
	virtual CallbackArgs callBack() = 0;
};
