...
namespace Audio {
    ...
    struct SimpleResampler::SimpleResamplerImpl {
        ...
        SimpleResamplerImpl( ) { std::memset( &data, 0, sizeof( SRC_DATA ) ); }
        ...
        SimpleResamplerImpl( float *dataIn, long input_frames, float *dataOut,
                             long outputFrames, double src_ratio )
            : data{ dataIn, dataOut, input_frames, 0, outputFrames,
                    0,      0,       src_ratio } {
            ;
        }
        ...
        void set( const float *data_in, float *data_out, long input_frames,
                  long output_frames, double ratio ) {
    ...
    struct FullResampler::FullResamplerImpl {
        ...
        FullResamplerImpl( int channels = 1, ResampleType converterType =
                                                 ResampleType::SINC_FASTEST )
            : channels( channels ), converterType( converterType ),
              hndl( HandleType(
                  src_new( int( converterType ), channels, &data.end_of_input ),
                  src_delete ) ) {
            std::memset( &data, 0, sizeof( SRC_DATA ) );
        }
        FullResamplerImpl( const FullResamplerImpl &cpy )
            : channels( cpy.channels ), converterType( cpy.converterType ),
              hndl( HandleType( src_new( int( cpy.converterType ), cpy.channels,
                                         &data.end_of_input ),
                                src_delete ) ) {
            std::memset( &data, 0, sizeof( SRC_DATA ) );
        }
        FullResamplerImpl( FullResamplerImpl &&mov ) : hndl( mov.hndl ) {
        ...
        void operator=( const FullResamplerImpl &cpy ) {
            ...
            this->hndl =
                HandleType( src_new( int( cpy.converterType ), cpy.channels,
                                     &data.end_of_input ),
                            src_delete );
            std::memset( &data, 0, sizeof( SRC_DATA ) );
        }
        ...
        int process( ) {
            if ( hndl )
                return src_process( hndl.get( ), &data );
            else
                return 0;
        }
        int reset( ) const {
            if ( hndl )
                return src_reset( hndl.get( ) );
            else
                return 0;
        }
        const float *getDataIn( ) const { return data.data_in; }
    ...
    struct CallbackResampler::CallbackResamplerImpl {
        ...
        CallbackResamplerImpl( CallbackResamplerImpl &&mov )
            : error( 0 ), channels( mov.channels ),
              converterType( mov.converterType ), hndl( mov.hndl ) {
            mov.hndl = nullptr;
        }
        ...
        CallbackResamplerImpl( const CallbackResamplerImpl &cpy ) =
            delete; // No copy-constructor
        void operator=( const CallbackResamplerImpl &cpy ) =
            delete; // No copy-assignment
        CallbackResamplerImpl( src_callback_t func, ResampleType converter_type,
                               int channels, void *userdata )
            : error( 0 ), channels( channels ), converterType( converter_type ),
              hndl( HandleType( src_callback_new( func, int( converter_type ),
                                                  channels, &error, userdata ),
                                src_delete ) ) {
            ;
        }
        CallbackResamplerImpl( CallbackInterfaceBase *interface,
                               ResampleType converter_type, int channels )
            : error( 0 ), channels( channels ), converterType( converter_type ),
              hndl( HandleType( src_callback_new( DefaultCallback,
                                                  int( converter_type ),
                                                  channels, &error, interface ),
                                src_delete ) ) {
            ;
        }
        long read( double src_ratio, long frames, float *data ) const {
            if ( hndl )
                return src_callback_read( hndl.get( ), src_ratio, frames,
                                          data );
            else
                return 0;
        }
    ...
    SimpleResampler::SimpleResampler( ) : pimpl( nullptr ) {}
    SimpleResampler::SimpleResampler( float *dataIn, long input_frames,
                                      float *dataOut, long outputFrames,
                                      double src_ratio )
        : pimpl( new SimpleResamplerImpl( dataIn, input_frames, dataOut,
                                          outputFrames, src_ratio ) ) {}
    SimpleResampler::SimpleResampler( const SimpleResampler &cpy )
        : pimpl( ( cpy.pimpl ) ? new SimpleResamplerImpl( *cpy.pimpl )
                               : nullptr ) {}
    ...
    void SimpleResampler::operator=( const SimpleResampler &cpy ) {
        pimpl = uSimpleResamplerImpl(
            ( cpy.pimpl ) ? new SimpleResamplerImpl( *cpy.pimpl ) : nullptr );
    }
    void SimpleResampler::operator=( SimpleResampler &&mov ) {
        this->pimpl = std::move( mov.pimpl );
    }
    FullResampler::FullResampler( ) : pimpl( nullptr ) { ; }
    ...
    FullResampler::FullResampler( const FullResampler &cpy )
        : pimpl( ( cpy.pimpl ) ? new FullResamplerImpl( *cpy.pimpl )
                               : nullptr ) {
        ;
    }
    ...
    void FullResampler::operator=( const FullResampler &cpy ) {
        pimpl = uFullResamplerImpl(
            ( cpy.pimpl ) ? new FullResamplerImpl( *cpy.pimpl ) : nullptr );
    }
    void FullResampler::operator=( FullResampler &&mov ) {
        this->pimpl = std::move( mov.pimpl );
    }
    CallbackResampler::CallbackResampler( ) : pimpl( nullptr ) {}
    CallbackResampler::CallbackResampler( CallbackResampler &&mov )
        : pimpl( std::move( mov.pimpl ) ) {}
    void CallbackResampler::operator=( CallbackResampler &&mov ) {
        this->pimpl = std::move( mov.pimpl );
    }
    CallbackResampler::CallbackResampler( samplerate_callback func,
                                          ResampleType converter_type,
                                          int channels, void *userdata )
        : pimpl( new CallbackResamplerImpl( func, converter_type, channels,
                                            userdata ) ) {}
    CallbackResampler::CallbackResampler( CallbackInterfaceBase *interface,
                                          ResampleType converter_type,
                                          int channels )
        : pimpl( new CallbackResamplerImpl( interface, converter_type,
                                            channels ) ) {}

    int SimpleResampler::resample( ResampleType resamplerType,
                                   int channels ) const {
        if ( pimpl )
            return pimpl->resample( resamplerType, channels );
        else
            return 0;
    }
    const float *SimpleResampler::getDataIn( ) const {
        if ( pimpl )
            return pimpl->getDataIn( );
        else
            return nullptr;
    }
    float *SimpleResampler::getDataOut( ) const {
        if ( pimpl )
            return pimpl->getDataOut( );
        else
            return nullptr;
    }
    long SimpleResampler::getInputFrames( ) const {
        if ( pimpl )
            return pimpl->getInputFrames( );
        else
            return 0;
    }
    long SimpleResampler::getOutputFrames( ) const {
        if ( pimpl )
            return pimpl->getOutputFrames( );
        else
            return 0;
    }
    ...
    void FullResampler::set( const float *data_in, float *data_out,
                             long input_frames, long output_frames,
                             double ratio ) const {
        if ( pimpl )
            return pimpl->set( data_in, data_out, input_frames, output_frames,
    ...
    long CallbackResampler::read( double src_ratio, long frames,
                                  float *data ) const {
        if ( pimpl )
            return pimpl->read( src_ratio, frames, data );
        else
            return 0;
    }
    ...
    ResampleType FullResampler::getResamplerType( ) const {
        if ( pimpl )
            return pimpl->converterType;
        else
            return ResampleType::SINC_BEST_QUALITY;
    }
    ...
    CallbackInterface::CallbackInterface( int channels,
                                          ResampleType converter_type )
